...
※DNSの割り当て変更の反映時間が読めない（最大で数日かかることも）ので、複数リレーを持っている人のみ推奨します。
...
２．ノードインストールを行う。2-7. gLiveViewのインストールまででOK。2-8. エアギャップオフラインマシンの作成は不要。
...
３．ブロックチェーンが同期されるのを待つ。２日～３日程度必要。
...
４．BPのgLiveViewにて、新Relayからの疎通確認をする。
・３が完了していなければ疎通が行われない。
・疎通がない場合は以下の項目を確認
...
５．DNSサーバーにてAレコードの割り当て変更を行う。割り当て変更の反映には数分～数日（とはいっても、おそらく６時間以内には反映される）かかる。
・割り当て変更が反映されたかの確認をするには
　BPにて
　nslookup <DNS>
　で表示された結果が新RelayのIPになっていれば、反映されている。

６．新RelayのgLiveViewにて、BPからの疎通確認をする。
・疎通がない場合は以下の項目を確認
